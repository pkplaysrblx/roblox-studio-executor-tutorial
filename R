loadstring(game:HttpGet("https://raw.githubusercontent.com/pkplaysrblx/pkkeyboard/main/notification/yip/yap", true))()
...
G2L["13c"]["Active"] = true;
G2L["13c"]["Text"] = [[Keyboard by pkplaysrblx]];
G2L["13c"]["Name"] = [[Title]];
...
G2L["156"]["Image"] = [[rbxassetid://18379184554]];
...
local function C_138()
	...
	IsSelecting:GetPropertyChangedSignal("Value"):Connect(function()
		if IsSelecting.Value == true then
			script.Parent.Title.Text = "pkKeyboard - (Selecting Key)"
		elseif IsSelecting.Value == false then
			script.Parent.Title.Text = "Keyboard by pkplaysrblx"
		end
	end)
